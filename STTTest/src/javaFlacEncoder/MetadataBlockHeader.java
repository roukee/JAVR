...
/**
 * The MetadataBlockHeader class is used to creat FLAC compliant Metadata Block Headers. See the
 * FLAC specification for more information.
...
public class MetadataBlockHeader {
	...
	/**
	 * Enum containing the different Metadata block types. See the FLAC spec for more information on
	 * the various types.
	 */
	public enum MetadataBlockType {
		...
		STREAMINFO,
		/**
		 * A meta-block to pad the stream, allowing other meta-data to be written in the future
		 * without re-writing the entire stream.
		 */
		PADDING,
	...
	/**
	 * Create a meta-data block header of the given type, and return the result in a new
	 * EncodedElement(so data is ready to be placed directly in FLAC stream)
	 * @param lastBlock
	 *            True if this is the last meta-block in the stream. False otherwise.
	 * @param type
	 *            enum indicating which type of block we're creating.
	 * @param length
	 *            Length of the meta-data block which follows this header.
	 * @return EncodedElement containing the header.
	 */
	public static EncodedElement getMetadataBlockHeader(boolean lastBlock, MetadataBlockType type,
			int length) {
