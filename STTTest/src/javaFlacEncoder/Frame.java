 ...
 * â€œchannel configurationâ€? used in encoding, such as mid-side or left-right(based upon the given
 * configuration).
...
public class Frame {
	 ...
	 * 'samples' may include both left and right samples of a stereo stream. Therefore, â€œskipâ€? would
	 ...
	 *         â€œ4000â€?. There is always an equal number of samples encoded from each channel. This
	...
	int encodeIndependent(int[] samples, int count, int start, int skip, EncodedElement result,
		...
		for (int i = 0; i < channelCount; i++) {
			int channelBitsPerSample = bitsPerSample;
	...
	private static int allChannelDecorrelation(int[] samples, int count, int start, int skip,
		...
		// int size = 0;
		/* sums[0]: left sums[1]: right sums[2]: mid sums[3]: side */
