...
public class CRC8 {
	...
	/**
	 * number of valid bits stored in workingCRC8(valid bits are packed towards high-order side of
	 * int
	 */
	...
	/**
	 * Constructor. Creates a CRC8 object that is ready to be used. Next step would be to call
	 * updateCRC8 with appropriate data.
	 */
	public CRC8() {
		reset();
	}
	...
	/**
	 * Add data to the crc. Immediately creates checksum on given data. Can be called multiple times
	 * as it won't finalize the checksum until the method checksum() is called.
	 * @param inSet
	 *            Array holding data to checksum.
	 * @param start
	 *            Index of array holding first element
	 * @param end
	 *            Index to stop at. Last index used will be end-1.
	 * @return intermediate result of checksum to this point. This is *not* a finalized result, as
	 *         non-summed data must remain in workingCRC8 until checksum() is called.
	 */
	...
	/**
	 * Finalize the checksum, and return the value. After this is called, you must call reset()
	 * before attempting a new checksum.
