...
public class RiceEncoder {
	...
	/**
	 * Constructor. A RiceEncoder object is used(as opposed to potentially faster static methods),
	 * for memory considerations; some temporary, dynamically created arrays are kept between calls
	 * to the encode methods to prevent frequently allocating and freeing similarly sized arrays.
	 */
	public RiceEncoder() {
	 ...
	 * @param useFiveBitParam
	 *            Set TRUE if using a five-bit parameter size, FALSE for a four-bit parameter
	 * @param order
	 *            Specify order of partitions to be used(actual number of partitions will be
	 *            2^order.
	 * @param ele
	 *            EncodedElement to write header to.
	...
	public static int encodeRicePartitionEscaped(int[] values, int inputOffset, int inputStep,
			int inputCount, EncodedElement destEle, int bitParam, boolean fiveBitParam) {
		if (DEBUG_LEV > 0)
			System.err.println("RiceEncoder::encode : Begin");
		/* Currently, we're passing in an EncodedElement with a set byte[], and filling that array.
		 * We should therefore ensure that we're not writing too much to it. We *can* add another
		 * element to the given one if need. */
	...
	/**
	 * Rice-encode a set of values, adding necessary headers for FLAC format. This encodes a single
	 * rice-partition. In general, beginResidual(...) should be used before this method.
	 * @param values
	 *            array of integer values to save
	 * @param inputOffset
	 *            start index in input array
	 * @param inputStep
	 *            number of values to skip between target values(for interleaved data.
	 * @param inputCount
	 *            number of total values to encode
	 * @param bitParam
	 *            rice-parameter to use. This value should be based upon the distribution of the
	 *            input data(roughly speeking, each value will require at least bitParam+1 bits to
	 *            save, so this value should reflect the average magnitude of input values.
	 * @param destEle
	 *            EncodedElement to save result to.
	 * @param fiveBitParam
	 *            Set true if this header should use a five-bit rice-parameter, false for a four bit
	 *            parameter.
	 * @return total encoded size(including headers)
	 */
	public int encodeRicePartition(int[] values, int inputOffset, int inputStep, int inputCount,
			EncodedElement destEle, int bitParam, boolean fiveBitParam) {
	...
	/* public int encodeRicePartitionDispatcher(int[] values, int inputOffset, int inputStep, int
	 * inputCount, EncodedElement destEle, int bitParam, boolean fiveBitParam) {
	 * 
	 * //return RiceEncoder.encodeRicePartitionOld(values, inputOffset, inputStep, inputCount,
	 * destEle, bitParam, fiveBitParam); //return RiceEncoder.encodeRicePartitionPackInt(values,
	 * inputOffset, inputStep, inputCount, destEle, bitParam, fiveBitParam); //return
	 * this.encodeRicePartitionObjectDest(values, inputOffset, inputStep, inputCount, destEle,
	 * bitParam, fiveBitParam); return 0; } */

	/**
	 * Calculate how large a given set of values will be once it has been rice-encoded. While this
	 * method duplicates much of the process of rice-encoding, it is faster than an actual encode
	 * since the data is not actually written to the flac bitstream format(a rather costly write).
	 * @param values
	 *            array of integer values to save
	 * @param inputOffset
	 *            start index in input array
	 ...
	 */
	public static int calculateEncodeSize(int[] values, int inputOffset, int inputStep,
			int inputCount, int bitParam) {
		// Pack int version of encode partition
		if (DEBUG_LEV > 0) {
