...
/**
 * FLAC_ConsoleFileEncoder is the command line interface to this FLAC encoder. It accepts several
 * flags and parameters to alter the encoding configuration.
...
public class FLAC_ConsoleFileEncoder {
	...
	/**
	 * flag to specify whether to use threads. Must follow by a zero(no threads) or a one(use
	 * threads)
	 */
	public static String THREADS = "-Threads";
	/**
	 * flag to specify subframe type to use. Must be followed by a valid type as given by the fields
	 * TYPE_*.
	 */
	...
	/**
	 * Exhaustive Subframe type identifier string(tells encoder to try each subframe type and choose
	 * the smallest one
	 */
	...
	public static final String THREADS_ERROR = "Error setting threads: \"" + THREADS
			+ "\" must be followed by an integer(max number of EXTRA" + " threads to use)";

	private File inputFile = null;
	private File outputFile = null;
	private final boolean canEncode = false;
	...
	/**
	 * Constructor. Uses given arguments to immediately attempts to encode the file given. Uses
	 * stdout and stderr for notices of progress and error.
	 * @param args
	 *            Encoding arguments to use.
	 */
	...
	private boolean getOutputFile(String filename) {
		...
		/*
		 * if(file.exists() ) { System.err.println("Output file exists, exiting"); result = false; } */
		{
			try {
				if (!file.exists())
					file.createNewFile();
	...
	/**
	 * Utility function to parse a positive integer argument out of a String array at the given
	 * index.
	 * @param args
	 *            String array containing element to find.
	 * @param index
	 *            Index of array element to parse integer from.
